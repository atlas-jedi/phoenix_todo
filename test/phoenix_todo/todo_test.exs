defmodule PhoenixTodo.TodoTest do
  ...
  describe "tasks" do
    ...
    @invalid_attrs %{description: nil, title: nil, completed_at: nil, created_at: nil, updated_at: nil}
    ...
    test "create_task/1 with valid data creates a task" do
      valid_attrs = %{description: "some description", title: "some title", completed_at: ~U[2024-06-10 14:10:00Z], created_at: ~U[2024-06-10 14:10:00Z], updated_at: ~U[2024-06-10 14:10:00Z]}

      assert {:ok, %Task{} = task} = Todo.create_task(valid_attrs)
      assert task.description == "some description"
      assert task.title == "some title"
      assert task.completed_at == ~U[2024-06-10 14:10:00Z]
      assert task.created_at == ~U[2024-06-10 14:10:00Z]
    ...
    test "update_task/2 with valid data updates the task" do
      task = task_fixture()
      update_attrs = %{description: "some updated description", title: "some updated title", completed_at: ~U[2024-06-11 14:10:00Z], created_at: ~U[2024-06-11 14:10:00Z], updated_at: ~U[2024-06-11 14:10:00Z]}

      assert {:ok, %Task{} = task} = Todo.update_task(task, update_attrs)
      assert task.description == "some updated description"
      assert task.title == "some updated title"
      assert task.completed_at == ~U[2024-06-11 14:10:00Z]
      assert task.created_at == ~U[2024-06-11 14:10:00Z]
